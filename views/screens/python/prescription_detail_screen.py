...
from views.screens.pyside.prescription_detail_screen_ui import PrescriptionDetailScreenUI, PrescriptionMedicineCard
from services.database_service import DatabaseService
from datetime import datetime
import os
import webbrowser
...
class PrescriptionDetailScreen(PrescriptionDetailScreenUI):
    ...
    def load_prescription_details(self):
        prescription = self.db.get_prescription_detail(self.prescription_id)
        if not prescription:
            QMessageBox.warning(self, "Lỗi", "Không tìm thấy đơn thuốc.")
            return
        self.prescription_name.setText(prescription.get('name', ''))
        self.hospital_name.setText(prescription.get('hospital_name', ''))
        self.created_date.setText(self.format_date(prescription.get('created_at')))
        status = prescription.get('status', 'active')
        if status == 'active':
            self.status_chip.setText("Đang điều trị")
            self.status_chip.setStyleSheet("font-size: 13px; border-radius: 8px; padding: 4px 12px; background: #ECFDF3; color: #12B76A;")
        elif status == 'completed':
            self.status_chip.setText("Đã hoàn thành")
            self.status_chip.setStyleSheet("font-size: 13px; border-radius: 8px; padding: 4px 12px; background: #ECFDF3; color: #12B76A;")
        else:
            self.status_chip.setText("Đã hủy")
            self.status_chip.setStyleSheet("font-size: 13px; border-radius: 8px; padding: 4px 12px; background: #FEF3F2; color: #F04438;")
        # Medicines
        ...
        for medicine in medicines_list:
            card = PrescriptionMedicineCard(medicine)
...
- {medicine.get('medicine_name', '')}
  ...
  + Hướng dẫn sử dụng: {medicine.get('usage_instruction', '')}
"""
        # Show in a dialog for copy/share
        QMessageBox.information(self, "Chia sẻ đơn thuốc", share_text)